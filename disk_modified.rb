...
class Rage::Deferred::Backends::Disk
  ...
  def initialize(path:, prefix:, fsync_frequency:)
    ...
    max_wal_timestamp = all_storages.flat_map do |storage|
      storage.tap(&:rewind).each_line(chomp: true).filter_map do |line|
        parts = line.split(":", 5)
        next unless parts.length >= 3
        parts[2].split("-").first.to_i
      end
    end.max.to_i

    # create seed value for the task IDs
    task_id_seed = [Time.now.to_i, max_wal_timestamp]. + 1 # TODO: ensure timestamps in the file are not higher
